...
def enhancedFeatureExtractorDigit(datum):
    ...
    fringe = util.Queue()
    fringe.push((0, 0)) #x, y
    whiteSpaces = util.Counter() # counting # of whiteSpaces
    while not fringe.isEmpty():
        (x, y) = fringe.pop()
        if datum.getPixel(x, y) > 0:
            continue
        if whiteSpaces[(x - 1, y)] == 0 and x - 1 > -1:
            fringe.push((x-1, y))
            whiteSpaces[(x-1, y)] = 1
        if whiteSpaces[(x + 1, y)] == 0 and x + 1 < DIGIT_DATUM_WIDTH:
            fringe.push((x+1, y))
            whiteSpaces[(x+1, y)] = 1
        if whiteSpaces[(x, y - 1)] == 0 and y - 1 > -1:
            fringe.push((x, y-1))
            whiteSpaces[(x, y-1)] = 1
        if whiteSpaces[(x, y + 1)] == 0 and y + 1 < DIGIT_DATUM_HEIGHT:
            fringe.push((x, y+1))
            whiteSpaces[(x, y+1)] = 1
    features["white"] = 0
    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT):
            if whiteSpaces[(x, y)] == 0 and datum.getPixel(x, y) == 0:
                features["white"] = 1
    return features
...
def enhancedPacmanFeatures(state, action):
    ...
    "*** YOUR CODE HERE ***"
    successor = state.generateSuccessor(0, action)
    foodLst = successor.getFood().asList()
    for food in foodLst:
        features[str(food)] = 1 / util.manhattanDistance(food, successor.getPacmanPosition())
    return features
