...
def normalize(factor):
    ...
    condVarSet = factor.conditionedVariables()
    uncondVarSet = factor.unconditionedVariables()
    for uncond in variableDomainsDict:
        if len(variableDomainsDict[uncond]) == 1:
            if uncond in uncondVarSet: #added this check
                uncondVarSet.remove(uncond)
                condVarSet.add(uncond)
    newFactor = Factor(uncondVarSet, condVarSet, variableDomainsDict)
    probSum = 0
