...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        A correct implementation will handle the following special case:
          *  When a ghost is captured by Pacman, all beliefs should be updated
             ...
             captured).
        """
        noisyDistance = observation #number
        emissionModel = busters.getObservationDistribution(noisyDistance) # dictionary? with a number with prob associated with it
        pacmanPosition = gameState.getPacmanPosition() #coordinate (x, y)

        "*** YOUR CODE HERE ***" #look into when u submitted projects and how many slip days u have left

        # Replace this code with a correct observation update
        # Be sure to handle the "jail" edge case where the ghost is eaten
        # and noisyDistance is None
        # implementing B(x_t+1) <- inf X_t+1 P(e_t+1 | X_t+1) * B'(X_t+1)
        allPossible = util.Counter()
        if noisyDistance is None:
            allPossible[self.getJailPosition()] = 1 #so ghost is here
        else:
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        """
        "*** YOUR CODE HERE ***"
        #This part is exact inference given time\
        #need pacman position?
        # B'(Xt+1)=summation_(x_t) Pr(Xt+1|xt)B(xt) implementing this
        allPossible = util.Counter()
        ...
        self.beliefs = allPossible # No need to normalize for time update
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        self.particles = []
        legal = self.legalPositions#before had self.legalPositions
        for i in range(self.numParticles):
            nextPos = legal[i % len(legal)]
            self.particles.append(nextPos)
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None: # case 1
            beliefs[self.getJailPosition()] = 1
        else:
            for particle in self.particles:
                beliefs[particle] += emissionModel[util.manhattanDistance(pacmanPosition, particle)] #particle represents ghost's pos
        if beliefs.totalCount() == 0: # case 2
            self.initializeUniformly(gameState)
        else:
            self.particles = []
            for i in range(self.numParticles):
    ...
    def elapseTime(self, gameState):
        ...
        for particle in range(len(self.particles)): 
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, self.particles[particle]))
            self.particles[particle] = util.sample(newPosDist)
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        for particle in range(numParticles):
            self.particles.append(possiblePos[particle % len(possiblePos)])
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for i in range(self.numGhosts):
                newDist = getPositionDistributionForGhost(setGhostPositions(gameState, list(oldParticle)), i, self.ghostAgents[i])
